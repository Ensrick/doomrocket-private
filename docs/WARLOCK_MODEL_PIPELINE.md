...
| v0.1.54-dev weapon-drop fix candidate | Keeps the v0.1.53 body/ragdoll implementation unchanged. Parents the loaded `pRocket` beneath the actor-owned `pRocketLauncher` while preserving its reviewed hand-space placement; no second dynamic actor is added | When AI inventory unlinks the death-dropped weapon and enables `rp_dropped`, both visible rigid meshes now live in that actor's transform subtree. Static and mutation tests reject the historical sibling topology | Require the exact v0.1.54 banner, body analyzer pass, and visual confirmation that the launcher and loaded warhead fall together. The flexible tube and chimney particles remain deliberately deferred |
