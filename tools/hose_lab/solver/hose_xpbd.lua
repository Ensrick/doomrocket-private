-- Isolated feasibility probe. Not loaded by the mod; no engine API calls.
...
function Solver.new(length, segments, damping, gravity)
    ...
    local self = setmetatable({length=length, segments=segments, n=segments+1,
        ...
        diagonal={}, upper={}, rhs={}, delta_lambda={}}, Solver)
...
function Solver:step(ax,ay,az,bx,by,bz)
    ...
    for i=1,self.n do
        ...
        if i>1 and i<self.n then
            x[i]=x[i]+(x[i]-self.px[i])*retention
            y[i]=y[i]+(y[i]-self.py[i])*retention
            z[i]=z[i]+(z[i]-self.pz[i])*retention+self.gravity*self.h*self.h
        end
    end
...
function Solver:frame(dt,ax,ay,az,bx,by,bz)
    ...
    for i=1,count do
        ...
        if not self:step(self.ax+(ax-self.ax)*fraction,self.ay+(ay-self.ay)*fraction,self.az+(az-self.az)*fraction,
            self.bx+(bx-self.bx)*fraction,self.by+(by-self.by)*fraction,self.bz+(bz-self.bz)*fraction) then
