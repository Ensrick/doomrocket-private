...
function Get-PublicationCommitSnapshot {
    ...
    $cfgPath = "$(Get-WarlockModPrefix $Mod)itemV2.cfg"
    $cfg = & $readBlob $cfgPath
    $cfgText = [System.Text.Encoding]::UTF8.GetString([byte[]]$cfg.Bytes)
    $sourceDescriptor = & $readBlob "$(Get-WarlockModPrefix $Mod)$Mod.mod"

    $bundlePrefix = "$(Get-WarlockModPrefix $Mod)bundleV2/"
    $bundles = @()
    ...
    $previewName = ''
    if ($configuredIsSafeLeaf -and $tree.ContainsKey("$(Get-WarlockModPrefix $Mod)$configured")) {
        $previewName = $configured
    }
    else {
        foreach ($candidate in @('item_preview.png', 'preview.jpg', 'preview.png')) {
            if ($tree.ContainsKey("$(Get-WarlockModPrefix $Mod)$candidate")) {
    ...
    $previewRepoPath = "$(Get-WarlockModPrefix $Mod)$previewName"
    ...
    $luaPath = "$(Get-WarlockModPrefix $Mod)scripts/mods/$Mod/$Mod.lua"
...
function New-WorkshopPublicationReceipt {
    ...
    if ($Repository -cne (Get-WarlockRepositoryProfile -RepoRoot $RepoRoot).Repository) {
        throw "Workshop publication receipt must match the selected standalone channel repository."
    }
    ...
    if ($authority -ceq 'receipt') {
        ...
        $result['bundle_authority_proof'] = [ordered]@{
            ...
            build_receipt_path = "$(Get-WarlockModPrefix $Mod).build-receipt.json"
