...
Assert-True ($itemConfig -match '(?m)^title\s*=\s*"Warprocket Bombardier TEST v0.1.71-dev";\s*$') `
    "development title must be exactly Warprocket Bombardier TEST v0.1.71-dev"
...
$reloadLifecycleRegression = Join-Path $PSScriptRoot 'tests\test_doomrocket_reload_lifecycle.py'
